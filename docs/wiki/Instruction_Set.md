...
Because this is a 64-bit architecture, the processor also supports 32-bit specific arithmetic operations (`OP-IMM-32` and `OP-32`). These are critical for C-compiled games like DOOM, as standard `int` variables in C are typically 32 bits. These instructions compute a 32-bit result and sign-extend it to 64 bits.

| Instruction | Name | Operation | Description |
| :--- | :--- | :--- | :--- |
| `addw` | Add Word | `rd = sign_extend((rs1 + rs2)[31:0])` | Adds two registers as 32-bit numbers and sign-extends the result. |
| `subw` | Subtract Word | `rd = sign_extend((rs1 - rs2)[31:0])` | Subtracts two registers as 32-bit numbers. |
| `addiw` | Add Immediate Word | `rd = sign_extend((rs1 + imm)[31:0])` | Adds a constant to a 32-bit register and sign-extends. |
